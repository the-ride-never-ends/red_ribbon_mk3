...
# TODO: Playwright is not being installed right, causing these tests to fail. Fix later.
...
async def patched_get(session, url, *args, **kwargs):
    try:
        ...
        raise FixtureError(f"Error in patched_get: {e}") from e
...
async def patched_get_html(url, *args, **kwargs):
    try:
        ...
        raise FixtureError(f"Error in patched_get_html: {e}") from e
