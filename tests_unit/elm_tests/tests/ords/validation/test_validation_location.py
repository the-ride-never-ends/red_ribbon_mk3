...
def testing_text_splitter():
    try:
        ...
        raise FixtureError(f"Error creating testing text splitter: {e}") from e
...
def oai_async_azure_client():
    ...
    try:
        ...
        raise FixtureError(f"Error creating Azure OpenAI client: {e}") from e


@pytest.fixture(scope="module")
def oai_async_client():
    """OpenAi Async client to use for tests"""
    try:
        return openai.AsyncOpenAI(
            api_key=os.environ.get("OPENAI_API_KEY"),
        )
    except Exception as e:
        raise FixtureError(f"Error creating OpenAI Async client: {e}") from e
...
def structured_llm_caller():
    ...
    try:
        ...
        raise FixtureError(f"Error creating StructuredLLMCaller: {e}") from e
...
async def test_url_matches_county(
    ...
    services = [
        OpenAIService(oai_async_azure_client, rate_limit=50_000)
    ]
...
async def test_doc_matches_county_jurisdiction(
    oai_async_azure_client, structured_llm_caller, county, doc_fp, truth, testing_text_splitter
):
...
async def test_doc_matches_county_name(
    oai_async_azure_client, structured_llm_caller, county, state, doc_fp, truth, testing_text_splitter
):
