...
if CSE_ID := os.getenv("GOOGLE_CSE_ID"):
    SE_TO_TEST.append(
        (elm.web.search.google.PlaywrightGoogleCSELinkSearch, {"cse_url": f"https://cse.google.com/cse?cx={CSE_ID}"})
    )

if os.getenv("GITHUB_ACTIONS") != "true":
    # Google link searches fail in GHA for some reason (but not locally)
    SE_TO_TEST += [
        (elm.web.search.google.PlaywrightGoogleLinkSearch, {}),
...
@pytest.mark.parametrize("queries", 
    [
        ...
        ["best way to learn Python", "how can I learn Python"],
    ]
)
