...
class VariableCodebook:
        ...
        Returns:
            dict[str, Any]: Operation results with the following structure:
                ...
                - Additional keys vary by action type:
                    - For "get_variable":
                        - 'variable' (Variable): Retrieved variable instance
                    - For "get_prompt_sequence":
                        - 'prompt_sequence' (list[str]): Generated prompt sequence
                    - For "get_assumptions":
                        - 'assumptions' (Assumptions): Retrieved assumptions
                    - For "add_variable" and "update_variable":
                        - 'variable' (Variable): Added or updated variable instance
                - If an operation fails, includes:
    ...
    def _extract_variable_from_input(self, input_data_point: str) -> str:
        ...
        Returns:
            Variable name
        """
        raise NotImplementedError("Variable extraction from input not implemented yet")
    ...
    def _get_variable(self, variable_name: str) -> dict[str, Any]:
        ...
        """
        output_dict = {
            "success": True if variable_name in self.variables else False,
            "variable": self.variables.get(variable_name, f"Variable not found: {variable_name}")
        }
        if output_dict["success"] == False:
