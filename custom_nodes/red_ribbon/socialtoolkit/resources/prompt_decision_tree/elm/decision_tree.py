...
class DecisionTree:
    ...
    def __init__(self, graph):
        ...
        """
        self._g: nx.DiGraph = graph
        self._history = []

        if 'api' not in self.graph.graph:
            msg = 'Graph must have the "api" attribute'
            logger.error(msg)
            raise AttributeError(msg)

        if not isinstance(self.graph, nx.DiGraph):
            msg = 'Graph must be a nx.DiGraph directed graph'
            logger.error(msg)
            raise AttributeError(msg)
    ...
    def _prepare_graph_call(self, node_name: str) -> str:
        ...
        txt_fmt = {
            k: v for k, v in self.graph.graph.items() if k != 'api'
        }
