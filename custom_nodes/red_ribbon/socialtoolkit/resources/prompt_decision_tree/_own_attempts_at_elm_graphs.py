...
_COMMENT_PROMPT = """
    ...
    of how you determined the value, if you think it is necessary (`null` otherwise).
"""
EXTRACT_ORIGINAL_TEXT_PROMPT = """
    "Can you extract the raw text with original formatting "
    "that states {extraction_query}? "
"""
...
def llm_response_starts_with_yes(response: str) -> bool:
    """Check if LLM response begins with "yes" (case-insensitive)"""
    return response.lower().startswith("yes")


def llm_response_starts_with_no(response: str) -> bool:
    """Check if LLM response begins with "no" (case-insensitive)"""
    return response.lower().startswith("no")


def llm_response_does_not_start_with_no(response: str) -> bool:
...
class ComfyWorkflow(BaseModel):
    ...
    links: list[Any] # int & str
    groups: Optional[list[Any]] # int & str
    config: dict[str, Any]


# def identify_node_type(node: ComfyNode) -> str:
#     """
#     Figure out whether or not a ComfyUI node should be a node, an edge, or an LLM output handler attribute in a nx.DiGraph
    
#     # Strategy outline:
#     # 1. Analyze node type (e.g., LLM nodes, condition nodes, etc.)
#     # 2. Check node outputs - nodes with LLMAPIRESPONSE outputs could be condition edges
#     # 3. Examine input/output connections - nodes that only pass through data might be edges
#     # 4. Consider semantic meaning - nodes representing decisions vs nodes representing data
#     # 5. Look at node properties and widget values for additional context
#     # 6. Return classification like "node", "edge", or specific subtypes
#     # 7. Special handling for nodes that represent branching conditions in decision trees
#     # 8. Consider position in overall workflow to determine role
#     """

# def identify_node_position_in_pipeline(node: ComfyNode, list_node_id: int) -> str:
#     """
#     Figure out which place in the pipeline a ComfyUI node was placed
#     """


# def comfy_pipeline_to_nx_digraph(pipeline_path: Path): # TODO
#     """Convert a ComfyUI pipeline to a NetworkX DiGraph. 
    
#     ```json
#         "links": [
#         [
#             4, # The ID of the nodes input
#             4, # The node's ID
#             0, # ????
#             5, # 
#             0, # Whether or not its the final node.
#             "LLMAPIRESPONSE"
#         ],
#     ```
...
#     comfy_workflow = ComfyWorkflow.model_validate(pipeline)

#     G = nx.DiGraph()
#     # Sort the nodes from by their order. Smallest means last.
#     nodes = sorted(comfy_workflow.nodes, key=lambda x: x.order)

#     for node in nodes:
#         node: ComfyNode
#         node_dict = {
#             "id": node.id,
#             "type": identify_node_type(node), # Can be "node", "edge", or "LLM output handler"
#             "order": node.order,
#             "widgets_values": node.widgets_values,
#         }
#         # If the node has no inputs, it's likely a utility node and should be discarded


#         # If the node has llm_api_response 
#         type_ = 
