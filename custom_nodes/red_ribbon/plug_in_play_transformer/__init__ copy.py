# import easy_nodes
# import os
# # NOTE This only needs to be called once.
# # easy_nodes.initialize_easy_nodes(default_category="Plug-in-Play-Transformer")

# # This must come after calling initialize_easy_nodes.
# from . import plug_in_play_transformer # noqa: E402
# # from . import model_layer_compare # noqa: E402

# # NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS = easy_nodes.get_node_mappings()

# # Export so that ComfyUI can pick them up.
#__all__ = ['NODE_CLASS_MAPPINGS', 'NODE_DISPLAY_NAME_MAPPINGS']

# # Optional: export the node list to a file so that e.g. ComfyUI-Manager can pick it up.
# # easy_nodes.save_node_list(
# #     os.path.join(os.path.dirname(__file__), "socialtoolkit_node_list.json")
# # )