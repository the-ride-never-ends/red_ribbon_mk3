...
def initialize_easy_nodes(default_category: str = "EasyNodes", 
    ...
    if _current_config:
        if _current_config.num_registered > 0: 
            ...
            if not ignore_non_fatal_errors:
                raise RuntimeError("Re-initializing EasyNodes, but no Nodes have been registered since last initialization. This may indicate an issue.")
    
        if _current_config.auto_register or not _current_config.NODE_CLASS_MAPPINGS:
            logging.warning(f"Auto-registration was turned off by previous initializer, but {len(_current_config.NODE_CLASS_MAPPINGS)} nodes were not picked up.")
            if not ignore_non_fatal_errors:
                raise RuntimeError(f"Auto-registration was turned off by previous initializer, but {len(_current_config.NODE_CLASS_MAPPINGS)} nodes were not picked up.")
...
def register_type(
    ...
    if not is_auto_register:
        if key not in _ANNOTATION_TO_COMFYUI_TYPE:
            print(f"WARNING: Type {cls} already registered. Skipping...")
    ...
    if name not in _COMFYUI_TYPE_TO_ANNOTATION_CLS:
        ...
        _custom_verifiers[name] = verifier if verifier is not None else SubclassVerifier(cls)
    elif verifier:
...
def show_text(text: str):
    ...
    if len(_curr_preview["text"]) > 0:
        if text.endswith((".", "!", "?", ":", ";", "...", "—")):
            ...
            _curr_preview["text"][-1] += text # Hopefully this should allow for streaming of text. # TODO Nope, it doesn't!
    else:
...
def _ensure_package_dicts_exist(module_name: str):
    ...
    package_name = module_name.split('.')[-2] if "." in module_name else module_name
...
def maybe_autoconvert(comfyui_type_name: str, arg: any):
    ...
    if _SHOULD_AUTOCONVERT.get(comfyui_type_name, False):
        ...
        arg = [comfyui_type(el) for el in arg] if isinstance(arg, list) else comfyui_type(arg)
...
def hex_to_color(color: str) -> list[float]:
    ...
    if len(col) != 6 or not all(c in "0123456789ABCDEF" for c in col):
        raise ValueError(f"Color must be a hex color code, got {color}")
    ...
    try:
        ...
        if color is not None:
            try:
                ...
                # Default to a light gray color if the provided color is invalid.
                logging.error(f"Invalid color {color} for node {cname}: {e}")
                color_rgb = [192, 192, 192]  # Light gray
        ...
        if bg_color is not None:
            try:
                ...
                logging.error(f"Invalid background color {bg_color} for node {cname}: {e}")
                bg_color = "#000000"
...
def _create_dynamic_setter(cls: type, debug=False) -> typing.Callable[..., T]:
    ...
    for attr_name in dir(obj):
        ...
        if attr is not None and not callable(attr) and not attr_name.startswith("__"):
            ...
            fully_qualified_name = _get_fully_qualified_name(prop_type)
            if "." in fully_qualified_name:
                all_type_names.add(fully_qualified_name)
