...
class RegisterRoutes:
    ...
    try: # Prevent easy node from erroring on start-up if PromptServer is not available
        routes = PromptServer.instance.routes
    except AttributeError:
        routes = web.RouteTableDef()
    except Exception as e:
        raise RuntimeError(f"Failed to get PromptServer routes: {e}") from e

    @routes.get('/easy_nodes/config.js')
    def api_get_user_config_file(request):
      """Returns the user configuration as a javascript file."""
      text=f'export const easy_node_config = {json.dumps(USER_CONFIG, sort_keys=True, indent=2, separators=(",", ": "))}'
...
api_get_user_config_file: Callable = RegisterRoutes.api_get_user_config_file
api_get_user_config:      Callable = RegisterRoutes.api_get_user_config
api_set_user_config:      Callable = RegisterRoutes.api_set_user_config